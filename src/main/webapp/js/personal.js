 ...
 * 地址页面添加名称至输入框
 */
$(".address_name .sp").click(function(){
	var value = $(this).html();
	$("#recvTag").val(value);
});
...
$(".save_recipient").click(function(){
	var receiverName = $("#recvName").val();// 收件人
	var receiverState = $("#recvProvince").val();// 省
	var receiverCity = $("#recvCity").val();// 市
	var receiverDistrict = $("#recvArea").val();// 区/县
	var receiverAddress = $("#recvAddr").val();//
...
function setDefault(e, id){
	console.log("走到setDefault中，id="+id);
	var parent = $(e).parent();
